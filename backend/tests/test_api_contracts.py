...
class BackendApiContractsTests(unittest.TestCase):
    ...
    def test_health_detailed_degrades_when_recommender_is_down(
        ...
        mock_check_http_health.side_effect = ["error: boom", "optional-unavailable: boom"]
    ...
    def test_dataset_users_proxies_recommender_payload(self, mock_get):
        ...
        response = self.client.get("/dataset-users?dataset=movielens&limit=5")
        self.assertEqual(response.status_code, 200)
        payload = response.json()
        ...
        response_v1 = self.client.get("/api/v1/datasets/movielens/users?limit=5")
